...
生成 5-8 条精选新闻 + 每条解读 + 当日综述，也支持结合历史指标的周度深度分析。

根目录 `.github/workflows/brief.yml` 默认仅允许手动运行，避免在信息源、模型输出和
飞书接收人尚未验收时自动消耗额度或对外推送。完成验收后，再由维护者显式加入定时触发。
...
仓库根目录 `.github/workflows/brief.yml` 默认没有 `schedule`。确认接收人和成本后，
可增加以下 cron（**注意是 UTC**）:
