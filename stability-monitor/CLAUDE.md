...
> ⚠️ **本文件是子目录的历史文档，权威性低于仓库根目录的 `CLAUDE.md` 与 `AUTOMATIONS.md`。**
> 它保留的是项目早期（2026-07-15 前后）的背景与设计意图，仍然有价值；但**当前评分、
> 定时任务、执行位置、告警逻辑一律以根目录两份文件为准**。两处冲突时以根目录为准。
>
...
军队渗透度、精英清洗计数、关键机构俘获度。
**重要口径注**: v3制度支柱高于v2(33)是重锚定结果非局势好转；weekly两期已按v3回溯。

> **分数已过时，勿引用本文件的数字。** 当前值以 `dashboard/data.js` 为准
> （截至 2026-07-30：财政48/货币40/制度37/社会55/强制37，综合43，weekly 5 期）。
...
- [x] 街头热度已接入"社会与街头"支柱的网络政治情绪 driver
- [x] 飞书 webhook + 部署：**已完成**。日/周/月全部跑在 GitHub Actions，推送统一走
      `.github/scripts/cloud_publish.py`；本机计划任务已于 2026-08-04 全部停用。
      看板交付为滚动 Release ZIP。细节见根目录 `AUTOMATIONS.md`。
