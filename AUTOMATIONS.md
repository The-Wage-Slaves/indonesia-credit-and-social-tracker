...
2. **周度风险卡**：周二统一运行信贷恐慌指数与街头热度。正常静默；红/橙风险、
   ...
   **观察时点段（2026-09-20 起）**：卡片尾部列出 `data.js` `watchlist` 里 30 天内到期与
   已过期未复核的项（读 `dashboard-data.json`，`apply_week.py export` 保持同步）。它只提醒
   日期、不判断事件、不改分。**第三种推送资格**：某项 7 天内到期或已过期未复核时，即使
   指数与街头热度都正常，周度卡也发送，标题写「观察时点提醒」而不是「待确认」——
   两者的决定不同：前者是「去核对结果并更新表」，后者是「裁定风险事件是否留痕」。
